...
my $usage = <<EOS;
   Usage: perl $0 -i inpath -b bampath [-options]
...
foreach my $file (sort {$a cmp $b} keys %files) {
   ...
   system("nohup other_scripts/draw_fusions.R --fusions='$outpath/tmp-dir/$sample_id.tsv' --alignments='$bampath/$sample_id.bam' --output='$outpath/$sample_id-fusions.pdf' --annotation=database/GENCODE19.gtf --cytobands=database/cytobands_hg19_hs37d5_GRCh37_v2.1.0.tsv --proteinDomains=database/protein_domains_hg19_hs37d5_GRCh37_v2.1.0.gff3 &"); 
