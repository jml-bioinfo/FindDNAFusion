...
my $usage = <<EOS;
   ...
   -i|inpatht	[string] (input dir storing fastq files at directory)
   -b|bedfile	[string] (input fusion range file in bed format)
   -o|outpath	[string] (output dir storng outputs from GeneFuse)
   -h|help (help information)

EOS

my ($inpath, $bedfile, $outpath, $help);
GetOptions (
  "inpath=s"  => \$inpath,        # input dir 
  "bedfile:s" => \$bedfile,	  # input fusion range file
...
if (!$bedfile) {
     $bedfile = "input_data/CTP-targeted-intron-regions.bed";
}
...
foreach my $file (@files) {
   ...
   if ($file =~ /^.+\/(.+)\.bam$/) {
       ...
       foreach my $bed_file (@bed_files) {
           ...
           system("nohup perl FACTERA/factera.pl -p 4 -s 2 -F -o $outpath3 $file FACTERA/exons.bed FACTERA/hg19.2bit $bed_file &");
      }
      system("sleep 3m");
   } 
}
