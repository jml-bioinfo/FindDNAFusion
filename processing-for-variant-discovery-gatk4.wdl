...
workflow PreProcessingForVariantDiscovery_GATK4 {
  ...
  String gatk_path = select_first([gatk_path_override, "/usr/local/lib/gatk-4.1.4.1/"])
  ...
  String gotc_path = select_first([gotc_path_override, "/usr/bin/"])
  String picard_path
  ...
  Array[File] flowcell_unmapped_bams_list
  ...
  scatter (unmapped_bam in flowcell_unmapped_bams_list) {
  ...
  # Copy output bam and index files
  call CopyFiles {
      input:
          inputFile1 = GatherBamFiles.output_bam,
          inputFile2 = GatherBamFiles.output_bam_index,
          outputPath = "${output_loc}"
  }
...
task CopyFiles {
  ...
  command {
    cp ${inputFile1} "${outputPath}"
    cp ${inputFile2} "${outputPath}"
  }

  output {
    String copied = outputPath
...
task BaseRecalibrator {
  ...
  command { 
    ${gatk_path}gatk --java-options "${java_opt}" \
      BaseRecalibrator \
      -R ${ref_fasta} \
      -I ${input_bam} \
